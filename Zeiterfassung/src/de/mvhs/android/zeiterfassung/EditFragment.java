...
public class EditFragment extends Fragment implements IRecordSelectedListener, LocationListener {
  ...
  public void onStart() {
    ...
    if (!_returnWithResult) {
      ...
      // Bilder
      _TakePicture = (Button) getActivity().findViewById(R.id.TakePicture);
      _SelectPicture = (Button) getActivity().findViewById(R.id.SelectPicture);
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (requestCode == 100 && resultCode == Activity.RESULT_OK) {
      ...
      Cursor imageData = getActivity().getContentResolver().query(selectedImage, pathColumn, null, null, null);
  ...
  private void loadData() {
    ...
    if (data != null && data.moveToFirst()) {
      ...
      if (!data.isNull(data.getColumnIndex(Columns.LONGTITUDE)) && !data.isNull(data.getColumnIndex(Columns.LATITUDE))) {
        _Position.setText(formatCoordinate(data.getDouble(data.getColumnIndex(Columns.LATITUDE)), data.getDouble(data.getColumnIndex(Columns.LONGTITUDE))));
      }
  ...
  private String formatCoordinate(double longitude, double latitude) {
    ...
    if (longitude != _NOT_SET && latitude != _NOT_SET) {
      ...
      formated = getString(R.string.CoordinateFormatString, longGrad, longMin, longSec, latGrad, latMin, latSec);
    }
